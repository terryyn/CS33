...
{
	int i;
#pragma omp parallel for num_threads(30) firstprivate(n, weights, arrayX, arrayY, xr, yr) private(i)
	for(i = 0; i < n; i++){
			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
	int i, j;
   	int index_X, index_Y;
	int max_size = X*Y*Z;
#pragma omp parallel for num_threads(30) firstprivate(arrayX, arrayY, seed, n) private(i)
   	for(i = 0; i < n; i++){
   		arrayX[i] += 1 + 5*rand2(seed, i);
   		arrayY[i] += -2 + 2*rand2(seed, i);
   	}
#pragma omp parallel for num_threads(30) firstprivate(Ones, n, arrayX, arrayY, objxy, iter, Y, Z, max_size, index, probability, array) private(i, j, index_X, index_Y)
   	for(i = 0; i<n; i++){
   		for(j = 0; j < Ones; j++){
...
#pragma omp parallel for num_threads(30) firstprivate(n, weights) private(i) reduction(+:sumWeights) 
   	for(i = 0; i < n; i++)
...
{
	int i, j;
#pragma omp parallel for num_threads(30) firstprivate(u, n, cfd, arrayX, arrayY, x_j, y_j) private(j, i)
	for(j = 0; j < n; j++){
...
#pragma omp parallel for num_threads(30)  firstprivate(n, arrayX, arrayY, x_j, y_j, weights) private(j)
	for(i = 0; i < n; i++){
		arrayX[i] = x_j[i];
		arrayY[i] = y_j[i];
		weights[i] = 1/((double)(n));
